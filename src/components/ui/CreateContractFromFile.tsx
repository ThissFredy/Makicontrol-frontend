...
import { uploadContractsFileService } from "@/services/contractService";
import { toast } from "react-hot-toast";
import { FiLoader, FiUploadCloud, FiX } from "react-icons/fi";

interface CreateContractsFormProps {
    onClose: () => void;
    onSuccess: (message: string) => void;
}

export const CreateContractsFromFile = ({
    onClose,
    onSuccess,
}: CreateContractsFormProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await uploadContractsFileService(formData);
        ...
        <form onSubmit={handleSubmit}>
            <div className="mb-6">
                <h2 className="text-2xl font-bold text-slate-800">
                    Crear Contratos desde Archivo
                </h2>
                <p className="mt-1 text-slate-500">
                    Sube un archivo Excel (.xlsx o .xls) para registrar
                    múltiples contratos a la vez.
                </p>
                <a
                    href="/files/contratos.xlsx"
                    download
