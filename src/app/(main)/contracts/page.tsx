...
const ContractsPage = () => {
            ...
            try {
                if (debouncedSearchTerms.query && debouncedSearchTerms.status) {
                    ...
                    if (response.status) {
                        ...
                        setContracts([]);
                        toast.error("No se encontraron contratos con esos criterios");
                    }
                ...
                if (debouncedSearchTerms.query && !debouncedSearchTerms.status) {
                    setLoadingContracts(true);
                    const response = await searchCustomerByNITService(debouncedSearchTerms.query);
                    if (response.status) {
                        setContracts(response.data);
                    } else {
                        setContracts([]);
                ...
                if (loading || (!debouncedSearchTerms.query && !debouncedSearchTerms.status)) {
                    setLoadingContracts(true);
                    ...
                    if (response.status) {
                        ...
                        if (loading) {
                            const activeCount = response.data.filter((c) => c.estado === "ACTIVO").length;
                            const inactiveCount = response.data.filter((c) => c.estado === "INACTIVO").length;
    ...
    const activeContractForMenu = contracts?.find(c => c.clienteNit === openMenuId);
                ...
                <div>
                    <div className="max-w-7xl mx-auto">
                        ...
                        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <StatCard icon={<FiFile size={24} />} title="Total Contratos" value={totalContracts} description="Contratos registrados" color="text-[#1A2541] " />
                            <StatCard icon={<FiCheck size={24} />} title="Contratos Activos" value={activeContracts} description="Contratos en estado activo" color="text-green-600" />
                            <StatCard icon={<FiX size={24} />} title="Contratos Inactivos" value={inactiveContracts} description="Contratos en estado inactivo" color="text-red-500" />
                        </section>

                        <main className="mt-8 bg-white p-6 rounded-lg shadow-sm border border-slate-200">
                            <div className="flex flex-col sm:flex-row items-center justify-between mb-6">
                                <div>
                                    <h2 className="text-xl font-bold text-slate-800">Lista de Contratos</h2>
                                    <p className="text-slate-500 text-sm mt-1">Busca y filtra tus contratos</p>
                                </div>
                                <div className="flex flex-wrap items-center gap-4 mt-4 sm:mt-0 w-full md:w-auto border border-slate-200 p-3 rounded-lg">
                                    <div className="relative w-full sm:w-100">
                                        ...
                                        <input type="text" placeholder="Buscar por NIT y Estado" value={searchTerms["query"] || ""}
                                            onChange={(e) => setSearchTerms({ ...searchTerms, query: e.target.value })}
                                    ...
                                    <div className="relative w-full sm:w-60 gap-2 flex items-center">
                                        ...
                                        <select value={searchTerms["status"] || ""} onChange={(e) => setSearchTerms({ ...searchTerms, status: e.target.value })}
                            ...
                            <div className="overflow-x-auto rounded-lg">
                                <table className="w-full text-md text-center text-slate-500">
                                    <thead className="text-xs text-white uppercase bg-[#253763]">
                                        <tr>
                                            <th scope="col" className="px-2 py-3"><div>NIT</div><div>Tipo de Contrato</div></th>
                                            <th scope="col" className="px-2 py-3">Valor Canon</th>
                                            <th scope="col" className="px-2 py-3">Valor Base Equipo</th>
                                            <th scope="col" className="px-2 py-3">Periodo</th>
                                            <th scope="col" className="px-2 py-3">Inicio / Fin</th>
                                            <th scope="col" className="px-2 py-3">Estado</th>
                                            <th scope="col" className="px-2 py-3">Canones</th>
                                    ...
                                    <tbody>
                                        {loadingContracts ? (
                                            <tr className="bg-white border-b">
                                                <td colSpan={10} className="px-2 py-4 text-slate-500">
                                                    <div className="flex items-center justify-center h-64">
                                                        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-blue-500"></div>
                                                    </div>
                                                </td>
                                            ...
                                            contracts?.length === 0 && (
                                                <tr className="bg-white border-b">
                                                    <td colSpan={10} className="px-2 py-4 text-slate-500">No hay contratos registrados.</td>
                                                </tr>
                                            )
                                        )}
                                        {contracts?.map((contract, index) => (
                                            <tr key={`${contract.clienteNit}-${index}`} className="bg-white border-b text-sm hover:bg-slate-50 text-[#000000]">
                                                <td className="px-2 py-4 font-medium">
                                                    <div className="text-lg">{contract.clienteNit}</div>
                                                    <div className="text-gray-500 font-normal">{contract.tipoContrato}</div>
                                                </td>
                                                <td className="px-2 py-4"><CurrencyInput id="valorCanon" name="valorCanon" value={contract.valorCanon} input={false} /></td>
                                                <td className="px-2 py-4"><CurrencyInput id="valorBase" name="valorBase" value={contract.valorBaseEquipo} input={false} /></td>
                                                <td className="px-2 py-4">{contract.periodo}</td>
                                                <td className="px-2 py-4"><div>{contract.fechaInicio}</div><div>{contract.fechaFin || "N/A"}</div></td>
                                                <td className="px-4 py-4 text-xs">
                                                    {contract.estado === "ACTIVO" ? (
                                                        <span className="text-green-800 bg-green-100 font-semibold border border-green-500 px-2 py-1 rounded-xl">{contract.estado}</span>
                                                    ) : (
                                                        <span className="text-red-800 bg-red-100 font-semibold border border-red-500 px-2 py-1 rounded-xl">INACTIVO</span>
                                                    )}
                                                </td>
                                                <td className="px-2 py-4 justify-center">
                                                    {contract.canones.length > 0 ? (
                                                        contract.canones.map((canon) => (
                                                            <div key={canon.grupo} className="flex justify-center gap-2">
                                                                <div className="text-[#000000] font-semibold">{canon.grupo}:</div>
                                                                <div className="text-slate-600 text-sm">{formatNumber(canon.valorCanon.toString())}</div>
                                                            </div>
                                                        ))
                                                    ) : (<div className="text-gray-400">No Aplica</div>)}
                                                </td>
                                                <td className="px-2 py-4 justify-center">
                                                    <div className="flex justify-center">
                                                        <div className="relative inline-block text-left">
                                                            ...
                                                            <button
                                                                ...
                                                                onClick={() => handleToggleMenu(contract.clienteNit, index)}
                    ...
                    <Modal isOpen={isModalOpen} onClose={handleCloseModal}><CreateContractForm onClose={handleCloseModal} onSuccess={handleCreationSuccess} /></Modal>
                    <Modal isOpen={isModalEditOpen} onClose={handleCloseModalEdit}><EditContractForm onClose={handleCloseModalEdit} onSuccess={handleEditationSuccess} initialData={selectedContract} /></Modal>
                    <Modal isOpen={isModalOpenFile} onClose={handleCloseModalFile}><CreateContractsFromFile onClose={handleCloseModalFile} onSuccess={handleFileUploadSuccess} /></Modal>
                    <Modal isOpen={isModalOpenDetailsFile} onClose={handleCloseModalDetailsFile}><CreateDetailsFromFile onClose={handleCloseModalDetailsFile} onSuccess={handleFileDetailsUploadSuccess} /></Modal>
                </div>
            )}

            {/* 5. Portal para el menú desplegable */}
            {openMenuId && activeContractForMenu && ReactDOM.createPortal(
                <div
                    ref={menuRef}
                    style={menuStyle}
                        ...
                        <div className="m-3 hover:scale-105 hover:font-bold transition-all duration-150">
                            <button onClick={() => { handleViewDetails(activeContractForMenu.clienteNit); closeMenu(); }} className="w-full text-slate-700 group flex items-center px-4 py-2 text-sm hover:cursor-pointer">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-slate-500 text-left">Inspeccionar detalles del contrato</div>
                                </div>
                            </button>
                        </div>
                        ...
                        <div className="m-3 hover:scale-105 hover:font-bold transition-all duration-150">
                            <button onClick={() => { handleOpenModalEdit(activeContractForMenu); closeMenu(); }} className="w-full text-slate-700 group flex items-center px-4 py-2 text-sm hover:cursor-pointer">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-slate-500 text-left">Modificar datos del contrato</div>
                                </div>
                            </button>
                        </div>
                    </div>
